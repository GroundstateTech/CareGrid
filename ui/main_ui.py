...
class CareGridApp(tk.Tk):
    ...
    def _build_quick(self,parent):
        tk.Label(parent,text='Quick Actions',bg=theme.HEADER,fg=theme.TEXT,font=('Segoe UI',10,'bold')).pack(fill=tk.X);body=tk.Frame(parent,bg=theme.PANEL);body.pack(fill=tk.BOTH,expand=True,padx=8,pady=8);ttk.Button(body,text='Add Files',command=self.add_files).pack(fill=tk.X,pady=3);self.live_btn=ttk.Button(body,text='Start Watch Folder',command=self.toggle_live);self.live_btn.pack(fill=tk.X,pady=3);ttk.Button(body,text='Settings',command=self.show_settings).pack(fill=tk.X,pady=3);ttk.Button(body,text='Silence 2 min',command=lambda:self.silence(120)).pack(fill=tk.X,pady=3);ttk.Button(body,text='Acknowledge All',command=self.ack_all).pack(fill=tk.X,pady=3)
